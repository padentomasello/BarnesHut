...
struct KernelArgs{
  cl_mem posxl, posyl, poszl, childl, massl, startl, minx_d, maxx_d, miny_d, maxy_d, minz_d, maxz_d, blocked, step_d, bottom_d, max_depth_d, radius_d;
...
void SetArgs(cl_kernel *kernel, KernelArgs* args){
  ...
  cl_int err;
  err = clSetKernelArg(*kernel, 0, sizeof(cl_mem), &args->posxl);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 1, sizeof(cl_mem), &args->posyl);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 2, sizeof(cl_mem), &args->poszl);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 3, sizeof(cl_mem), &args->childl);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 4, sizeof(cl_mem), &args->massl);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 5, sizeof(cl_mem), &args->startl);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 6, sizeof(cl_mem), &args->minx_d);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 7, sizeof(cl_mem), &args->maxx_d);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 8, sizeof(cl_mem), &args->miny_d);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 9, sizeof(cl_mem), &args->maxy_d);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 10, sizeof(cl_mem), &args->minz_d);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 11, sizeof(cl_mem), &args->maxz_d);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 12, sizeof(cl_mem), &args->blocked);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 13, sizeof(cl_mem), &args->step_d);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 14, sizeof(cl_mem), &args->bottom_d);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 15, sizeof(cl_mem), &args->max_depth_d);
  CHK_ERR(err);
  err = clSetKernelArg(*kernel, 16, sizeof(cl_mem), &args->radius_d);
  CHK_ERR(err);
...
int main (int argc, char *argv[])
{
  ...
  args.step_d = clCreateBuffer(cv.context, CL_MEM_READ_WRITE,
      sizeof(int) * 1, NULL, &err);
  args.bottom_d = clCreateBuffer(cv.context, CL_MEM_READ_WRITE,
      sizeof(int) * 1, NULL, &err);
  args.max_depth_d = clCreateBuffer(cv.context, CL_MEM_READ_WRITE,
      sizeof(int) * 1, NULL, &err);
  args.radius_d = clCreateBuffer(cv.context, CL_MEM_READ_WRITE,
      sizeof(float) * 1, NULL, &err);
  // Create Buffers  NOTE* These do need to be (num_nodes + 1)


  args.posxl = clCreateBuffer(cv.context, CL_MEM_COPY_HOST_PTR | CL_MEM_READ_WRITE,
      sizeof(float) * (num_nodes + 1), host_memory.posx, &err);
  CHK_ERR(err);
  args.poszl = clCreateBuffer(cv.context, CL_MEM_COPY_HOST_PTR | CL_MEM_READ_WRITE,
      sizeof(float)*(num_nodes + 1), host_memory.posy, &err);
  args.posyl = clCreateBuffer(cv.context, CL_MEM_COPY_HOST_PTR | CL_MEM_READ_WRITE,
      sizeof(float)*(num_nodes + 1), host_memory.posz, &err);
  CHK_ERR(err);
  args.massl = clCreateBuffer(cv.context, CL_MEM_READ_WRITE,
      sizeof(float)*(num_nodes + 1), NULL, &err);
  CHK_ERR(err);
  countl = clCreateBuffer(cv.context, CL_MEM_READ_WRITE,
      sizeof(float)*(num_nodes + 1), NULL, &err);
  CHK_ERR(err);
  args.startl = clCreateBuffer(cv.context, CL_MEM_READ_WRITE,
      sizeof(int)*(num_nodes + 1), NULL, &err);
  CHK_ERR(err);
  args.childl = clCreateBuffer(cv.context, CL_MEM_READ_WRITE,
  ...
  int inc = (num_bodies + WARPSIZE -1) & (-WARPSIZE);
    ...
    //velxl = clCreateSubBuffer(args.childl, CL_MEM_READ_WRITE,
        //CL_BUFFER_CREATE_TYPE_REGION, &velxl_region, &err);
    //velyl = clCreateSubBuffer(args.childl, CL_MEM_READ_WRITE,
        //CL_BUFFER_CREATE_TYPE_REGION, &velyl_region, &err);
    //velzl = clCreateSubBuffer(args.childl, CL_MEM_READ_WRITE,
        //CL_BUFFER_CREATE_TYPE_REGION, &velzl_region, &err);
    //accxl = clCreateSubBuffer(args.childl, CL_MEM_READ_WRITE,
        //CL_BUFFER_CREATE_TYPE_REGION, &accxl_region, &err);
    //accyl = clCreateSubBuffer(args.childl, CL_MEM_READ_WRITE,
        //CL_BUFFER_CREATE_TYPE_REGION, &accyl_region, &err);
    //acczl = clCreateSubBuffer(args.childl, CL_MEM_READ_WRITE,
        //CL_BUFFER_CREATE_TYPE_REGION, &acczl_region, &err);
    //sortl = clCreateSubBuffer(args.childl, CL_MEM_READ_WRITE,
